...
namespace AppInventario.Services
{
    public class PessoaService
    {
        ...
        {
            var pessoas = await _context.Pessoas.Include(p => p.Propriedades).Where(p => p.Id == id). FirstOrDefaultAsync();
            return pessoas;
        }

        public async Task <Pessoa> GetPessoa(string cpf)
        {
            var pessoas = await _context.Pessoas.Include(p => p.Propriedades).Where(p => p.Cpf == cpf).FirstOrDefaultAsync();
            return pessoas;
        }
